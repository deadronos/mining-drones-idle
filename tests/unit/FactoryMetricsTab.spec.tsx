...
describe('ui/FactoryMetricsTab', () => {
  ...
  it('renders metric cards and allows pausing sampling', async () => {
    ...
    const metricsSeries = {
      oreIn: [{ ts: now, value: 180 }],
      barsOut: [{ ts: now, value: 90 }],
      energyUse: [{ ts: now, value: 7.5 }],
      haulerThroughput: [{ ts: now, value: 360 }],
    };
    ...
    act(() => {
      storeApi.setState((state) => ({
        ...
        metrics: {
          ...
          snapshots: { [factory.id]: { ore: 0, bars: 0, energy: 0, timestamp: now } },
          pendingHauler: {},
          accumulatorMs: 0,
        },
      }));
