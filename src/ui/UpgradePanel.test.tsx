...
import { storeApi, PRESTIGE_THRESHOLD, type Resources, type Modules, type Prestige } from '@/state/store';
...
describe('ui/UpgradePanel - prestige button', () => {
  ...
  it('enables the Prestige Run button when bars cross the threshold', async () => {
    ...
    storeApi.setState((s) => ({ resources: { ...s.resources, bars: PRESTIGE_THRESHOLD + 100 } }));

    await waitFor(() => expect(screen.getByRole('button', { name: /Prestige Run/i })).not.toBeDisabled());
  });
});
